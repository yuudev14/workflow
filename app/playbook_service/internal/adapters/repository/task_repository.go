...
import (
	...
	"github.com/lib/pq"
	"github.com/yuudev14/ytsoar/db/queries"
...
func (t *TaskRepositoryImpl) CreateTaskHistory(tx *sqlx.Tx, workflowHistoryId string, tasks []domain.Tasks, graph map[uuid.UUID][]uuid.UUID) ([]domain.TaskHistory, error) {
	...
	return DbExecAndReturnMany[domain.TaskHistory](
		tx,
		statement,
	)
}
...
func (t *TaskRepositoryImpl) GetTasksByPlaybookId(workflowId string) []domain.Tasks {
	result, _ := DbExecAndReturnManyOld[domain.Tasks](
		t,
...
func (t *TaskRepositoryImpl) UpsertTasks(tx *sqlx.Tx, workflowId uuid.UUID, tasks []domain.Tasks) ([]domain.Tasks, error) {
	...
	return DbExecAndReturnMany[domain.Tasks](
		tx,
		statement,
	)
}
...
func (t *TaskRepositoryImpl) UpdateTaskStatus(workflowHistoryId string, taskId string, status string) (*domain.TaskHistory, error) {
	...
	return DbExecAndReturnOne[domain.TaskHistory](
		t.DB,
		statement,
	)
}
...
func (t *TaskRepositoryImpl) UpdateTaskHistory(workflowHistoryId string, taskId string, taskHistory tasks.UpdateTaskHistoryData) (*domain.TaskHistory, error) {
...
// GetTaskHistoryByPlaybookHistoryId implements PlaybookRepository.
func (t *TaskRepositoryImpl) GetTaskHistoryByPlaybookHistoryId(id string, filter tasks.TaskHistoryFilter) ([]domain.TaskHistory, error) {
...
// GetTaskHistorGetTaskHistoryCountyByPlaybookId implements PlaybookRepository.
func (w *TaskRepositoryImpl) GetTaskHistoryCount(filter tasks.TaskHistoryFilter) (int, error) {
	statement := sq.Select("count(*)").From("task_history")
	if filter.PlaybookID != nil {
		statement = statement.Where("workflow_history_id = ?", filter.PlaybookID)
