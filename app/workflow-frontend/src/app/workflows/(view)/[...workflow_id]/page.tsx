...
const Page: React.FC<{ params: Promise<{ workflow_id: string }> }> = ({
  ...
  const nodes: Node<PlaybookTaskNode>[] = useMemo(() => {
    if (
      ...
      !Array.isArray(workflowQuery.data.tasks)
    )
      return [];
  ...
  const edges = useMemo(() => {
    if (
      workflowQuery.data === undefined ||
      !Array.isArray(workflowQuery.data.edges)
    )
      return [];
      ...
      .map((value) => {
        if (["created_at", "updated_at"].includes(value)) {
          return {
            ...
            value: lib.utils.readableDate(
              currentNode[value as keyof Tasks] as string
            ),
          };
        }
        return {
                ...
                <AccordionItem value="parameters">
                  ...
                  <AccordionContent className="flex flex-col gap-4">
                    <div className="flex flex-col gap-2">
                      <Label className="font-normal">Parameter 1</Label>
                      <Input disabled value="asas" />
                    </div>
                    <div className="flex flex-col gap-2">
                      <Label className="font-normal">Parameter 1</Label>
...
const RenderKeyValue: React.FC<{ label: string; value: string }> = ({
  label,
  value,
}) => {
  return (
