...
CREATE TABLE IF NOT EXISTS playbook_history (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4 (),
    playbook_id UUID REFERENCES playbooks (id) ON DELETE CASCADE,
    status playbook_status NOT NULL DEFAULT 'in_progress',
    error TEXT,
    result JSONB,
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    CONSTRAINT unique_playbook_name UNIQUE (playbook_id, name)
);
...
CREATE TABLE IF NOT EXISTS task_history (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4 (),
    playbook_history_id UUID REFERENCES playbook_history (id) ON DELETE CASCADE,
    task_id UUID,
    name VARCHAR(50),
    description TEXT,
    parameters JSONB,
